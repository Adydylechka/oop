...
public final class FlatsController {
    private static Flat tableToUpdate;
    private DTO dto;
    ...
    private static ArrayList<DTO> convert(List<Flat> tables) {
        ArrayList<DTO> dtos = new ArrayList<>();
        for (Flat table : tables) {
            dtos.add(new DTO(table.getArea(), table.getNumberOfLegs(), table.getMaker().toString()));
        }
        return dtos;
    }

    public static Flat getTableToUpdate() {
